...
| :-- | :--- | :--- | :--- | :--- |
| 1 | `İsmail Yılmaz bakiyesi` | `cari_durum`, `musteri = "İsmail"` — soyadı **düşer** | Okuma akışındaki `_terim_cikar`, `SORU_SOZLUGU` köküne 3 harften itibaren yaslanır; `YILMAZ` = `YIL` + 3 harflik ek (tolerans 6) olduğundan durak kelime sayılıp elenir. Yazma akışı (`tahsilat_coz`) kökü ≥4 harfle sınırlar ve `"İsmail Yılmaz"`ı korur; iki akış burada ayrışır. Aynı sınıf (ölçüldü): `Günay` (`GUN`), `Sonat` (`SON`); `Aydın` KORUNUR çünkü `AY` iki harflidir ve kök eşlemesi ≥3 harf ister. | `tests/test_wa3_niyet.py::test_OKUMA_akisinda_YILMAZ_soyadi_YIL_kokune_yaslanir_OLCULDU` |
