...
@pytest.mark.asyncio
async def test_conflicting_cookie_headers_are_rejected_before_authentication() -> None:
    sent, downstream_calls = await _execute(
        [
            (b"cookie", b"erp_access=first-token"),
            (b"cookie", b"erp_access=second-token"),
        ]
    )

    assert downstream_calls == 0
    assert sent[0]["status"] == 400
    assert _response_payload(sent)["code"] == "AMBIGUOUS_COOKIE"
    assert (b"cache-control", b"no-store") in sent[0]["headers"]


@pytest.mark.asyncio
async def test_identical_duplicate_cookie_headers_are_rejected() -> None:
    sent, downstream_calls = await _execute(
        [
            (b"Cookie", b"erp_access=same-token"),
            (b"cookie", b"erp_access=same-token"),
        ]
    )
...
@pytest.mark.asyncio
async def test_single_cookie_header_remains_compatible() -> None:
    sent, downstream_calls = await _execute(
        [(b"cookie", b"erp_access=token; csrf_token=proof")]
    )
...
@pytest.mark.asyncio
async def test_cookie_ambiguity_precedes_authorization_evaluation() -> None:
    sent, downstream_calls = await _execute(
        [
            (b"cookie", b"erp_access=first-token"),
            (b"cookie", b"erp_access=second-token"),
