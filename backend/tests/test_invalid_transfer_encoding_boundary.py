...
@pytest.mark.parametrize(
    "value",
    [b"", b"gzip", b"identity", b"gzip, chunked", b"chunked;level=1"],
)
async def test_invalid_transfer_encoding_is_rejected_before_body_parsing(
    value: bytes,
) -> None:
    sent, downstream_calls = await _execute([(b"transfer-encoding", value)])

    assert downstream_calls == 0
    assert sent[0]["status"] == 400
    assert _payload(sent)["code"] == "INVALID_TRANSFER_ENCODING"
    assert (b"cache-control", b"no-store") in sent[0]["headers"]


@pytest.mark.asyncio
@pytest.mark.parametrize("value", [b"chunked", b" CHUNKED "])
async def test_chunked_transfer_encoding_remains_compatible(value: bytes) -> None:
    sent, downstream_calls = await _execute([(b"transfer-encoding", value)])
...
@pytest.mark.asyncio
async def test_duplicate_transfer_encoding_still_has_ambiguity_precedence() -> None:
