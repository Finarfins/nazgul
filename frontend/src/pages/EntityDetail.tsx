...
export default function EntityDetail({type}:{type:EntityType}){
  ...
  // CS4 — formda cek/senet yalniz `payments` ile gorunur; tahsilat penceresi
  // zaten ayni kapinin arkasinda, liste de oradan besleniyor.
  const formMethods=useMemo(()=>Object.entries(paymentLabel).filter(([v])=>canPayments||!cekYontemiMi(v)),[canPayments]);
  ...
  // yontemlerde evrak alanlarini zorunlu kildi (422). CS3'un evrak penceresi
  // on dolgulu acilir; odeme satirini ters kopru (`payment_olustur`) yazar.
  const savePayment=async()=>{try{setFormError('');if(paymentAmount<=0)throw new Error('Geçerli tutar girin.');if(cekYontemiMi(paymentMethod)){setCekAcik(true);return}await api.post('/payments',{entity_type:type,entity_id:entityId,amount:paymentAmount,payment_date:paymentDate,note:paymentNote||null,payment_method:paymentMethod,account_id:paymentAccountId||null});setPaymentOpen(false);load()}catch(err:unknown){setFormError(errorDetail(err,'Tahsilat veya ödeme kaydedilemedi.'))}};
