...
import {api,errorDetail,openAuthenticated} from '../api';
...
export default function InventoryCountsReport(){
 ...
 const threshold=Math.max(0,Number(minVariance)||0);
 const visibleRows=useMemo(()=>rows.filter(row=>{
  if(varianceFilter==='unchanged'&&Number(row.changed_count||0)!==0)return false;
  return Number(row.total_absolute_variance||0)>=threshold;
 }),[rows,varianceFilter,threshold]);
 ...
 const resetFilters=()=>{setWarehouseId('');setDateFrom('');setDateTo('');setVarianceFilter('all');setMinVariance('');};
 const exportCsv=()=>{
 ...
 const exportReport=async()=>{
  try{
   ...
   if(varianceFilter==='changed')params.set('changed_only','true');
   if(threshold>0)params.set('min_absolute_variance',String(threshold));
   const query=params.toString();
   await openAuthenticated(`/exports/warehouse-count-variance.xlsx${query?`?${query}`:''}`,'stok-sayim-farklari.xlsx');
  }catch(e){
   setError(errorDetail(e,'Stok sayım fark raporu indirilemedi.'));
 ...
 return <Stack spacing={2}>
  <Stack direction={{xs:'column',md:'row'}} justifyContent="space-between" gap={1.5}>
   ...
   <Stack direction={{xs:'column',sm:'row'}} spacing={1} sx={{alignSelf:{xs:'stretch',md:'flex-start'}}}>
    ...
    <Button variant="outlined" startIcon={<DownloadIcon/>} onClick={exportReport} disabled={exporting}>{exporting?'Rapor Hazırlanıyor...':'Excel’e Aktar'}</Button>
  ...
  <Card><CardContent>
   <Stack direction={{xs:'column',md:'row'}} spacing={1} flexWrap="wrap" useFlexGap>
    ...
    <TextField size="small" type="number" label="Asgari belge mutlak farkı" value={minVariance}
     onChange={e=>setMinVariance(e.target.value)} slotProps={{htmlInput:{min:0,step:'any'}}} sx={{minWidth:{md:205}}}/>
  ...
  <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
   ...
   {threshold>0&&<Chip color="info" label={`Belge önem eşiği: ${threshold.toLocaleString('tr-TR')}`}/>} 
  </Stack>
